...
def fftWorker(fft_audio_in_queue, frequency_dist_queue, RATE, RATE_FREQUENCY, NUM_PIXELS):
    ...
    frequency_dist = np.array([0 for j in range(120)]) # wir gehen davon aus, dass max_key < 120
    if max_key >= 120:
    ...
    while True:
        ...
        if np.any(frequency_dist):
            if np.max(frequency_dist) != 0:
                frequency_dist_normalized = 1/np.max(frequency_dist)*frequency_dist
                frequency_dist = frequency_dist_normalized
