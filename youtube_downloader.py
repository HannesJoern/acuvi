...
class youtube_downloader:
    def __init__(self,path) -> None:
        ...
        self.real_downloaded_file_names = []
        self.url_list = []
        path_ = self.path+'\\wav_files'
        onlyfiles = [f for f in listdir(path_) if isfile(join(path_, f))]
        onlyfiles.remove('wav_files_readme.txt')
        for i in onlyfiles:
            filename, file_extension = os.path.splitext(i)
            self.downloaded_file_names.append(filename)
    ...
    def download(self):
        ...
        stream_url = yt_dlp.YoutubeDL({
            ...
            'outtmpl': self.path + '\\wav_files\\%(title)s.%(etx)s',
            'quiet': False
        })
        info = stream_url.extract_info(url, download=False)
        file_name = str(stream_url.prepare_filename(info))
        file_name = file_name.split("\\")
        file_name ,ext= os.path.splitext(file_name[len(file_name)-1])
